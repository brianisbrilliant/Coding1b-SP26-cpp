...
public: 
    ...
    baseSlime() {
        ...
        health = rand() % 4 + 4;
        startingHealth = health;            // save this for eventual boss slime creation
        damage = rand() % 3 + 3;
    }
...
public:
    ...
    // this runs after the baseSlime constructor nuts
    bossSlime() {
...
class splitSlime : public baseSlime {
    // build constructor to use half-starting health of original slime
    // on death, add myself to a vector of "dead slimes"
        // that can be regenerated into a boss slime.
};
